...
declare variable $processConcept:input := collection($config:data-root || "/concepts");
  declare variable $processConcept:thotVariables := doc($config:data-root || "/thot-config.xml");
  declare variable $processConcept:schemes := doc($config:data-root || "/schemes/external-schemes.rdf");
  declare variable $processConcept:requests := collection($config:data-root ||"/requests");
...
let $broaderTerms:=
 ...
 let $narrowerTerms:=
                <div class="panel panel-default panel-terms">
                             <div class="panel-heading">
                                <h2 class="panel-title"> Narrower Terms<span class="skosLabel"> (skos:narrower)<a title="" data-html="true" data-toggle="popover" data-content="" data-original-title="skos:
                                                  Narrower"><i class="glyphicon glyphicon-question-sign
                                  skosQuestion"></i></a></span></h2>
                             </div>
                             <div class="panel-body">
                          ...
                          <span class="dateEntry">
                            {if(starts-with($temp/periodo:earliestYear, '-'))
                              ...
                              (
                                ...
                                then
                                  (
                                    concat(substring($temp/periodo:earliestYear, 2), " - ", substring($temp/periodo:latestYear, 2), " ", $processConcept:thotVariables//item[@type='bc'][@xml:lang=$language])
                                  )
                                  else if($temp/periodo:latestYear >0)
                                  then
                                    (
                                      concat(substring($temp/periodo:earliestYear, 2), " ", $processConcept:thotVariables//item[@type='bc'][@xml:lang=$language], " - ", $temp/periodo:latestYear, " ", $processConcept:thotVariables//item[@type='ad'][@xml:lang=$language])
                                    )
                                    else()
                              )
                            else(
                              concat($temp/periodo:earliestYear, " - ", $temp/periodo:latestYear, " ", $processConcept:thotVariables//item[@type='ad'][@xml:lang=$language])
                            )
...
{if (sm:has-access (xs:anyURI($config:modules-root || '/4access.xql') , 'r-x' ))
            ...
            <div class="h4 pull-right"><span class="pastilleLabelBlue pastilleURI">Status </span>{data($concept//thot:admin/@status)}</div>
            </div>
            )else()}
...
if($concept//thot:admin/@status="deleted") then
    (
      <p>deleted</p>
    )
    else if(not($concept))
...
else if($schemeNode/thot:admin/@status = "published")
    then
                (
