...
declare function local:relatedDocuments($resourceUri as xs:string){
    let $collection := collection("/db/apps/" || $project || "Data/documents")
    let $relatedDocs:= $collection//tei:TEI[descendant-or-self::tei:listPlace//tei:place[functx:contains-any-of(tei:placeName/@ref || " ", $resourceUri || " ")]]
                            |$collection//tei:TEI[descendant-or-self::tei:history/tei:origin/tei:origPlace[functx:contains-any-of(./@ref || " ", $resourceUri || " ")]]
    let $docs:=
        for $doc in $relatedDocs
                  ...
                  let $docUri := if($doc//tei:fileDesc/tei:publicationStmt/tei:idno[@type="uri"]) then $doc//tei:fileDesc/tei:publicationStmt/tei:idno[@type="uri"]
                                                                                         else $teiEditor:data-repository-path || "/documents" || $docId
          return <li>{ $title } {if($placeRelationType != "") then " [" || $placeRelationType || "]" else ("")}
          { $dating }<a href="{ $docUri }" target="about"><i class="glyphicon glyphicon-new-window"/></a></li>
    ...
    return
            ...
            <span class="subSectionTitle">{ $label }</span>
               <div>
                    <ul>{ $docs[position() < 6] }
...
let $placeNodeToInsert := <nodeToInsert>
    <place>
        <placeName ref="{$data//placeUri/text()}" ana="provenance">{$data//placeName/text()}</placeName>
        </place>

    </nodeToInsert>

let $updatePlaceList :=  update insert functx:change-element-ns-deep($placeNodeToInsert, "http://www.tei-c.org/ns/1.0", "")/node() into
                 util:eval( "doc('" || $doc-collection-path ||"/" || $filename
                            ||"')")//tei:sourceDesc/tei:listPlace
let $updateProvenanceUri := update value util:eval( "doc('" || $doc-collection-path ||"/" || $filename
                            ||"')")//tei:fileDesc/tei:sourceDesc/tei:msDesc/tei:history/tei:provenance/tei:location/tei:placeName/@ref with $data//placeUri/text()
        
let $updateProvenanceName := update value util:eval( "doc('" || $doc-collection-path ||"/" || $filename
                            ||"')")//tei:fileDesc/tei:sourceDesc/tei:msDesc/tei:history/tei:provenance/tei:location/tei:placeName with $data//placeName/text()
...
let $logEvent := teiEditor:logEvent("document-new" , $newDocId, (),
    ...
    <result><newDocId>{ $newDocId }</newDocId>
    <sentData>{ $data }</sentData>
    <newList>{ local:relatedDocuments($data//placeUri/text()) }</newList>
    </result>
)
