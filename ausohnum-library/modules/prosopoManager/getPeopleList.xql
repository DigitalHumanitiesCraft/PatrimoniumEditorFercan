...
declare variable $list:= doc("/db/apps/" || $project || "Data/lists/list-people.xml");

$list/peopleList/root
...
(\:doc("/db/apps/" || $project || "Data/lists/people.xml")/root:\)
...
declare variable $peopleCollection := collection("/db/apps/" || $project || "Data/people");
 <root>{
  for $person in $peopleCollection//lawd:person[@rdf:about != ""]
      ...
      let $exactMatches :=  
                for $exactMatch in $person//skos:exactMatch
                    return 
      ...
      let $personFunctions :=
            for $function in distinct-values(data($person//apc:hasFunction/@rdf:resource))
                return 
                '<a href="' || $function || '" target="about" class="label label-primary labelInTable" title="Open record in new tab" 
                style="">' || functx:capitalize-first(ausohnumCommons:getLabelFromConcept($function, $lang)) || '</a>'
...
(\:                    ($name || ' <a href="' || substring-before($uri, "#this") || '" target="blank"><i class="glyphicon glyphicon-new-window"/></a>' ):\)
                    else " "
                    }</name>
