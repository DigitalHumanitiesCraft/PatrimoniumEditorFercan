...
/*                    var resource = url + "#this";*/
                if (resource == ""){var concept2display = "intro"; var resource="intro"}
                  else
...
/*                    $("#collection-tree").fancytree("getTree").activateKey(thotNoUni);*/
            ...
            var activeNode = $("#collection-tree").fancytree("getTree").getActiveNode();
             ...
             if(concept2display == "new") {var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=newPlaceForm"}
             else if (resource == "intro") { var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=placeIntro" }
             else if (resource != null && activeNode == null) {
             var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=noPlaceFound&resource=" + url; 
/*             var sourceFromXql = "$ausohnum-lib/modules/handleRequests.xql?type=noPlaceFound&resource=" + url;*/
             }
             
             else {
                     activeNode.makeVisible({scrollIntoView: true});
                     
/*             var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=getPlaceHTML&resource=" + encodeURIComponent(activeNode.data.uri);}*/
         var sourceFromXql = "$ausohnum-lib/modules/handleRequests.xql?type=getPlaceHTML&resource=" + encodeURIComponent(activeNode.data.uri);}
         $("#placeEditor").load(sourceFromXql);
            var nodeTitle=activeNode.title;
            document.title = "Place - " + nodeTitle;
...
/*             var source2 = templatepath + nodeId2Use + "&lang=" + lang;*/
             if(nodeId == "new") {var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=newPlaceForm"}
             else {
             console.log(encodeURIComponent(nodeUri));
/*             var sourceFromXql = "$ausohnum-lib/modules/spatiumStructor/getFunctions.xql?type=getPlaceHTML&resource=" + encodeURIComponent(nodeUri);*/
             var sourceFromXql = "/getfunction/?type=getPlaceHTML&resource=" + encodeURIComponent(nodeUri);
             }
...
/*             console.log("node URI= " + nodeUri + " - Key = " + nodeKey);*/
             history.pushState(null, null,  "/places/" + nodeId);
