...
/*                    $("#collection-tree").fancytree("getTree").activateKey(thotNoUni);*/
            ...
            var activeNode = $("#collection-tree").fancytree("getTree").getActiveNode();
             ...
             if(concept2display == "new") {var sourceFromXql = "$ausohnum-lib/modules/prosopoManager/getFunctions.xql?type=newPersonForm"}
             else {
             activeNode.makeVisible({scrollIntoView: true});
/*             var sourceFromXql = "$ausohnum-lib/modules/prosopoManager/getFunctions.xql?type=getPeopleHTML&resource=" + encodeURIComponent(activeNode.data.uri);*/
             var sourceFromXql = "/getfunction/?type=getPeopleHTML&resource=" + encodeURIComponent(activeNode.data.uri);
             }
            $("#peopleEditor").load(sourceFromXql);
            var nodeTitle=activeNode.title;
            document.title = "People " + " - " + nodeTitle;
...
/*             var source2 = templatepath + nodeId2Use + "&lang=" + lang;*/
             if(nodeId == "new") {var sourceFromXql = "$ausohnum-lib/modules/prosopoManager/getFunctions.xql?type=newPersonForm"}
             else {
/*             var sourceFromXql = "$ausohnum-lib/modules/prosopoManager/getFunctions.xql?type=getPeopleHTML&resource=" + encodeURIComponent(nodeUri);*/
             var sourceFromXql = "/getfunction/?type=getPeopleHTML&resource=" + encodeURIComponent(nodeUri);
             }
...
/*             console.log("node URI= " + nodeUri + " - Key = " + nodeKey);*/
             history.pushState(null, null,  "/people/" + nodeId);
...
renderNode: function(event, data) {
    ...
    if (node.data) {
        ...
        });//End of FancyTree collectionTree
        
        

    $("#peopleTree").fancytree({
        extensions: ["glyph", "filter"],
        glyph: glyph_opts,
        clickFolderMode: 3,
        minExpandLevel: 2,
...
/*                    $("#collection-tree").fancytree("getTree").activateKey(thotNoUni);*/
            ...
            $("#peopleTree").fancytree("getTree").activateKey(concept2display);
            
            var activeNode = $("#peopleTree").fancytree("getTree").getActiveNode();
...
/*        $( ".nodeTitleen" ).addClass( "displayedNodeTitle" );        */
       ...
       activate: function(node) {
            ...
            var treeroot = $("#peopleTree").fancytree("getRootNode").getFirstChild()
            var lang = treeroot.data.lang;
            console.log("language in tree: " + lang);
            var tree = $("#peopleTree").fancytree("getTree")
            var activeNode = tree.getActiveNode();
...
/*             var sourceFromXql = "$ausohnum-lib/modules/prosopoManager/getFunctions.xql?type=getPeopleHTML&resource=" + encodeURIComponent(nodeUri);*/
             var sourceFromXql = "/getfunction/?type=getPeopleHTML2&resource=" + encodeURIComponent(nodeUri);
             }
